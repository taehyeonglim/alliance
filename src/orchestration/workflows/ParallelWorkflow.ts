...
export class ParallelWorkflow extends BaseWorkflow {
  readonly type: WorkflowType = 'parallel';

  /** Optional merger agent for result aggregation */
  private mergerAgent?: IAgent;
  ...
  async execute(context: WorkflowContext): Promise<WorkflowResult> {
    const executionPath: string[] = ['parallel_start'];
  ...
  private createBranchContext(context: WorkflowContext, agent: IAgent): AgentContext {
    return {
      state: context.state,
      invocation: {
        ...context.invocation,
  ...
  private createMergerContext(
    ...
    const mergerInput = Object.fromEntries(
      Array.from(results.entries()).map(([id, result]) => [id, result.output])
    );
  ...
  private combineOutputs(results: Map<string, AgentResult>): Record<string, unknown> {
