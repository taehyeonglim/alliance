...
exports.manualCleanupTrash = exports.cleanupTrash = exports.restartWorkflow = exports.processApproval = exports.onWorkflowCreated = void 0;
...
async function getGeminiAI() {
    ...
    if (!apiKey) {
        try {
            ...
            if (configDoc.exists) {
                apiKey = (_a = configDoc.data()) === null || _a === void 0 ? void 0 : _a.gemini_api_key;
            }
        }
        catch (error) {
 ...
 * 워크플로우 생성 시 자동 실행
 */
exports.onWorkflowCreated = functions
    ...
    WORKFLOW_PHASES.forEach((phase) => {
        phase.agents.forEach((agentId) => {
            ...
            steps.push({
                ...
                status: 'idle',
                isLoopAgent: ((_a = phase.loopAgents) === null || _a === void 0 ? void 0 : _a.includes(agentId)) || false,
 ...
 * 에이전트 실행
 */
async function executeAgent(workflowId, agentId, context) {
 ...
 * 승인 게이트 처리
 */
async function handleApprovalGate(workflowId, agentId, content) {
    ...
    await db.collection('approvals').add({
        workflowId,
        workflowName: (workflow === null || workflow === void 0 ? void 0 : workflow.name) || 'Unknown',
        agentId,
 ...
 * 다음 에이전트로 진행
 */
async function proceedToNextAgent(workflowId, currentAgentId) {
    const workflowDoc = await db.collection('workflows').doc(workflowId).get();
    const workflow = workflowDoc.data();
    if (!workflow)
        return;
 ...
 * 다음 Phase로 진행
 */
async function proceedToNextPhase(workflowId) {
    const workflowDoc = await db.collection('workflows').doc(workflowId).get();
    const workflow = workflowDoc.data();
    if (!workflow)
        return;
 ...
 * 에이전트 컨텍스트 빌드
 */
function buildAgentContext(workflow) {
    return {
        researchTopic: workflow.researchTopic,
        name: workflow.name,
        ...workflow.results,
 ...
 * 진행률 계산
 */
function calculateProgress(workflow, currentAgentId) {
 ...
 * 워크플로우 단계 상태 업데이트
 */
async function updateWorkflowStep(workflowId, agentId, status) {
    const workflowDoc = await db.collection('workflows').doc(workflowId).get();
    const workflow = workflowDoc.data();
    if (!workflow)
        return;
    const steps = workflow.steps || [];
    const updatedSteps = steps.map((step) => step.agentId === agentId ? { ...step, status } : step);
    await db.collection('workflows').doc(workflowId).update({
        steps: updatedSteps,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
 ...
 * 로그 추가
 */
async function addLog(workflowId, level, agentId, message) {
 ...
 * 승인 처리
 */
exports.processApproval = functions
    .region('asia-northeast3')
    ...
    if (action === 'approve') {
        ...
        if (agentId === 'research-question-confirm' && (phaseConfig === null || phaseConfig === void 0 ? void 0 : phaseConfig.loopAgents)) {
            const workflowDoc = await db.collection('workflows').doc(workflowId).get();
            const workflow = workflowDoc.data();
            await db.collection('workflows').doc(workflowId).update({
                currentAgent: phaseConfig.loopAgents[0],
                phaseIteration: ((workflow === null || workflow === void 0 ? void 0 : workflow.phaseIteration) || 1) + 1,
            });
            await addLog(workflowId, 'info', 'system', `🔄 수정 요청 - 아이디어 정교화 ${((workflow === null || workflow === void 0 ? void 0 : workflow.phaseIteration) || 1) + 1}차 반복`);
 ...
 * 워크플로우 재시작
 */
exports.restartWorkflow = functions
    .region('asia-northeast3')
    .https.onCall(async (data, context) => {
 ...
 */
exports.cleanupTrash = functions
    .region('asia-northeast3')
    .pubsub
 ...
 */
exports.manualCleanupTrash = functions
    .region('asia-northeast3')
    .https.onCall(async (data, context) => {
