...
export class LoopWorkflow extends BaseWorkflow {
    ...
    constructor(id, name, agents, config, terminationCondition) {
    ...
    async execute(context) {
        ...
        while (iteration < maxIterations) {
            ...
            for (const agent of this.agents) {
                const iterationContext = this.createIterationContext(context, agent, iteration, currentOutput);
                try {
                    ...
                    if (iterationContext.invocation.actions.escalate) {
                        ...
                        return this.createResult(true, agentResults, executionPath, undefined, currentOutput, iteration);
                    }
                    // Human intervention check
                    if (result.requiresReview) {
                        const approval = await this.requestApproval(context, agent, result.output);
                        if (!approval.approved) {
                            return this.createResult(false, agentResults, executionPath, 'Human rejected', undefined, iteration);
                        }
                    }
                }
                catch (error) {
    ...
    createIterationContext(context, _agent, iteration, input) {
        return {
            state: context.state,
            invocation: {
                ...context.invocation,
...
//# sourceMappingURL=LoopWorkflow.js.map